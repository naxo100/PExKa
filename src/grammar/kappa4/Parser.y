...
%code top {
	...
	#define yylex(x) x.getNextToken<kappa4::Parser::symbol_type>()
...
%token SIGNATURE INIT LET CONST ASSIGN ASSIGN2 
%token AT ATD FIX OP_CUR CL_CUR OP_PAR CL_PAR OP_BRA CL_BRA COMMA DOT TYPE LAR KAPPA_RAR JOIN FREE
%token PERT DO UNTIL INTRO DELETE SET PLOT OBS TRACK CPUTIME CONFIG REPEAT DIFF PRINT PRINTF STOP SNAPSHOT
